...
- Rewrite color/theme imports to
  `@workspace/zawadi-hr-design-system/lib/native-theme` and
  `@workspace/zawadi-hr-design-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/zawadi-hr-design-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/zawadi-hr-design-system` and retains only
