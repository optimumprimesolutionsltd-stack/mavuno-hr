...
workspace package (`@workspace/mavuno-hr-design-system`): other artifacts depend
...
- `docs/references/logos/mavuno-mark.svg` — the retained Mavuno geometric mark,
...
`@workspace/mavuno-hr-design-system/components/...`. Never use a `@/` alias inside
...
This extraction preserves the source app's canonical dark cockpit language:
emerald is reserved for primary action and positive emphasis; navy-black
surfaces and low-contrast borders create the dense operational canvas; Inter
handles UI copy; JetBrains Mono is reserved for identifiers, amounts, dates,
and other operational data. Use the tight 0.3rem radius and the emerald/cyan/
blue chart progression rather than introducing decorative gradients or loose
rounded cards. The light mode is a deliberate counterpart for consumers that
need it, not the source app's primary presentation.

## Prototyping on the canvas

Use the mockup-sandbox skill's "Design systems" flow. It creates a sandbox entry
for `@workspace/mavuno-hr-design-system` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/mavuno-hr-design-system` as a `workspace:*` dependency,
run `pnpm install`, and import directly from this package.

Read only the guides required by the current task:

- Building or styling web UI: `artifacts/mavuno-hr-design-system/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/mavuno-hr-design-system/docs/consuming-expo.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/mavuno-hr-design-system/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/mavuno-hr-design-system/docs/migrating-expo.md`
