...
async function pollPendingMpesaPayments(): Promise<void> {
  ...
  for (const payment of pending) {
    ...
    try {
      ...
      const { accessUntil } = await applyVerifiedPayment({
        orgId: payment.orgId,
        paymentId: payment.id,
        amountCents: payment.amount,
        actor: { userId: null, email: "mpesa-poller@system", ip: null },
        detail: `recovered M-Pesa payment ${receiptNo}`,
      });

      logger.info(
        { paymentId: payment.id, orgId: payment.orgId, accessUntil },
