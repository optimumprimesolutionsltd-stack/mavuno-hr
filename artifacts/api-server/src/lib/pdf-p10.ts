...
export function generateP10Pdf(cards: P10CardData[]): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      layout: "landscape",
      margin: 0,
      info: { Title: "P10 Tax Deduction Card", Author: "Mizani HR", Subject: "KRA P10" },
    });
    ...
    for (const card of cards) {
      ...
      for (const note of notes) {
       ...
       doc.font("Helvetica").fontSize(6).fill("#667085")
          .text(`Generated by Mizani HR  •  P10 ${card.year}  •  Employee ${card.employee.empNo}`, PL, doc.page.height - 18, { width: PW, align: "right" });
    }
