...
export async function calculateRun(
  ...
  const slipRows = staff.map((e) => {
    ...
    const pin: PayInput = {
      ...
      overtimeHours: org.overtimeEnabled ? (ts?.overtimeHours ?? 0) : 0, holidayHours: ts?.holidayHours ?? 0,
      daysInPeriod,
    ...
    return {
      ...
      helb: r.helb, sacco: r.sacco, insurancePremium: r.insurancePremium, loanDeduction: r.loanDeduction,
